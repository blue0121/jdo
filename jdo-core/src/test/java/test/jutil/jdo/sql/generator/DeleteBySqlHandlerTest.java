...
public class DeleteBySqlHandlerTest extends SqlHandlerTest {
    ...
    public void testSql1() {
	    ...
	    Assertions.assertEquals("delete from usr_group where name=?", response.getSql());
	...
	public void testSql2() {
		...
		handler.handle(request, response);
		System.out.println(response.getSql());
		Assertions.assertEquals("delete from usr_group where name=? and count=?", response.getSql());
